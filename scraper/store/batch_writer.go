...
func NewBatchWriter(outDir string) (*BatchWriter, error) {
	...
	w.SetKeyValueMetadata("schema", "raw_training_row_v1")
...
type ArchiveBatchWriter struct {
	outDir string
	tmpDir string
	...
	file   *os.File
	writer *parquet.GenericWriter[ArchiveTurnRow]

	bufferedGames int
	bufferedRows  int
}

func NewArchiveBatchWriter(outDir string) (*ArchiveBatchWriter, error) {
	if outDir == "" {
		return nil, fmt.Errorf("outDir is required")
	}
	...
	w := parquet.NewGenericWriter[ArchiveTurnRow](
		f,
		parquet.Compression(&zstd.Codec{Level: zstd.SpeedBetterCompression}),
	)
	w.SetKeyValueMetadata("schema", "archive_turn_v1")

	return &ArchiveBatchWriter{
		outDir:  absOut,
		tmpDir:  tmpDir,
		name:    name,
		tmpPath: tmpPath,
...
func (b *ArchiveBatchWriter) TmpPath() string    { return b.tmpPath }
func (b *ArchiveBatchWriter) OutPath() string    { return b.outPath }
func (b *ArchiveBatchWriter) BufferedGames() int { return b.bufferedGames }
func (b *ArchiveBatchWriter) BufferedRows() int  { return b.bufferedRows }

func (b *ArchiveBatchWriter) WriteRows(rows []ArchiveTurnRow) error {
	if b.writer == nil || b.file == nil {
		return fmt.Errorf("archive batch writer is closed")
	}
	if len(rows) == 0 {
		return nil
	}
...
func (b *ArchiveBatchWriter) NoteGameWritten() {
	b.bufferedGames++
}

func (b *ArchiveBatchWriter) Finalize() (outPath string, rows int, games int, err error) {
	if b.writer == nil && b.file == nil {
		return "", 0, 0, nil
	}
